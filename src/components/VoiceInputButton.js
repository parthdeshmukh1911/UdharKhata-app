...
const LANGUAGE_MAP = {
  ...
  mai: 'mai-IN',
  sat: 'sat-IN',
  bho: 'hi-IN', // or a suitable locale close to Bhojpuri
  mrw: 'mr-IN', // or suitable Marwadi locale if exists
  sd: 'sd-IN',  // if available, else fallback to Hindi or Urdu locale
};
...
export default function VoiceInputButton({ 
  ...
  const { currentLanguage } =
    ENABLE_I18N ? useContext(SimpleLanguageContext) : { currentLanguage: 'en' };
  ...
  sd: 'مثال:\nکریڈٹ: "فاطمہ کي 500 ڏنو"\nادائيگي: "علي کان 1000 مليو"',
    ...
    if (matchedCustomers.length === 0) {
      showAlert({
        ...
        message: `No customer "${parsed.customerName}" found in your contacts.\n\nPlease add them manually first, then try voice input again.`,
        type: 'warning',
        buttons: [{ text: 'OK', style: 'primary' }],
      });
      return;
    ...
    if (matchedCustomers.length > 1) {
      // Use CustomerSelectModal instead of stuffing all names into an alert
  ...
  sd: 'مثال: "گراهڪ انور نمبر 9876543210 شامل"',
    ...
    showAlert({
      ...
      message: `Name: ${parsed.customerName}\nPhone: ${parsed.phoneNumber}\n\nPlease confirm and save.`,
      type: 'success',
      buttons: [
        { text: 'Cancel', style: 'secondary' },
        {
  ...
  const addTransactionForCustomer = async (customer, parsed) => {
    ...
    showAlert({
      ...
      message: `Adding ${parsed.transactionType.toLowerCase()} of ₹${parsed.amount} for ${customer['Customer Name']}\n\nPlease review and confirm.`,
      type: 'success',
      buttons: [{ text: 'OK', style: 'primary' }],
    });
  };
        ...
        style={[
          {
            ...
            borderRadius: 28,
            backgroundColor:
              isListening ? '#ef4444' : theme.colors.secondary || '#10b981',
      ...
      <CustomerSelectModal
        ...
        subtitle={
          pendingParsed?.customerName
            ? `Matches for "${pendingParsed.customerName}"`
            : undefined
        }
        onSelect={(customer) => {
